...
def main():
    ...
    body = (LEAD("Önce modelin ne olduğunu sade anlatalım; sonra teknik kuralları ve varsayımları kaynaklarıyla verelim.",
     ...
     + H3("Model nedir? (sade)","What is the model? (plain)")
     + P("Modelimiz bir bilgisayar simülasyonu — bir tür kalabalık “video oyunu”. Ekrandaki her insan bir noktadır ve üç basit kurala uyar: <strong>(1)</strong> hedefine/çıkışına doğru yürür; <strong>(2)</strong> başkalarına ve duvarlara çarpmaz; <strong>(3)</strong> etrafındaki görünmez “stres havasını” (duygu alanı) sezip kararını — örneğin hangi kapıya gideceğini — değiştirir. Bu stres, zeminde görünmez bir ısı haritası gibi birikir, çevreye yayılır ve zamanla söner. Kurallar binlerce minik adımda tekrarlanınca, kimse merkezden yönetmeden, gerçek kalabalık davranışları (sıkışma, kuyruk, kilitlenme, panik) kendiliğinden ortaya çıkar.",
        "Our model is a computer simulation — a kind of crowd “video game”. Each person on screen is a dot following three simple rules: <strong>(1)</strong> walk toward your goal/exit; <strong>(2)</strong> don’t bump into others or walls; <strong>(3)</strong> sense the invisible “stress atmosphere” around you (the affect field) and change your decision — e.g. which door to take. That stress builds like an invisible heat-map on the floor, spreads to neighbours, and fades over time. Repeating the rules over thousands of tiny steps, with no central controller, real crowd behaviours (congestion, queues, jamming, panic) emerge on their own.")
     + NOTE("tip","Bu tür modele ne denir?","What is this kind of model called?",
        "Buna <em class=\"term\">ajan-tabanlı simülasyon</em> denir: tek tek bireyleri (ajanları) basit kurallarla canlandırır, topluluk davranışının kendiliğinden çıkmasını izlersin. Üstüne koyduğumuz özgün parça, stresin <em>kişide değil ortamda</em> tutulduğu “duygu alanı”dır. Simülasyonu canlı görmek için 3B oynatıcıya bakabilirsin (Bölüm 6).",
        "This is an <em class=\"term\">agent-based simulation</em>: you animate individuals (agents) with simple rules and watch collective behaviour emerge. Our novel part is the “affect field” that holds stress <em>in the environment, not in the person</em>. To see it live, view the 3D player (Section 6).")
